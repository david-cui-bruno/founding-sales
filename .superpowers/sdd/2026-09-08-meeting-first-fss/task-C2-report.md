...
- `status(pairingId)` and `revokeGoogleGrant(pairingId)` return `GrantStatus = {state:'unconfigured'|'ready'|'revoked',grant:GoogleGrant|null,providerRevocation?:'confirmed'|'pending'}`. Status never includes tokens. Remote Google revocation first invalidates durable local grant/state, then attempts Google token revocation. A definitive HTTP failure remains pending and explicitly retryable with encrypted tokens inaccessible to execution. An ambiguous provider-response exception retains its in-flight hold and is not automatically retryable.
...
## Independent review repairs (historical implementation checkpoint)

Read `task-C2-review.md` in full. Reviewer I2 was reproduced before repair with three behavioral REDs: failed revocation still allowed begin, a held provider revoke still allowed begin, and a competing revoke reported confirmed while the first request remained in flight.

I2 implementation is committed **`80015a558e64785e4f07074121c207cc481a8914`**, exactly `remoteGoogleAuthorization.ts` and its test via `git commit --only --`, paths inspected. Regrant now requires confirmed prior revocation. A durable CAS-owned `revocationInFlight` flag permits only one active revocation request. Known returned failures retain encrypted cleanup material and allow explicit retry, not regrant. Competing revoke callers report pending without making a second request or clearing the first claim. An old OAuth callback cannot replace pending/confirmed cleanup. Crash/ambiguous claim commit remains conservatively pending without automatic lease takeover; explicit operational reconciliation is required before activation/regrant in that case. This is a safety hold, not a new cloud operation or provider-scope assumption.

I2 GREEN: **25 OAuth tests**, strict scoped TypeScript and two-file lint/diff checks passed. Added actual emitted-transaction tests for failed-revoke→regrant, held revoke versus begin/callback, duplicate active revoke, held OAuth exchange completing after revocation, and lost claim response without provider call/automatic takeover. Existing API/status types are unchanged. `google_revocation_pending` is a static internal refusal, redacted by the existing HTTP boundary. No C1/helper/sender edits.

I1 reproduced through the actual local provider manager and encrypted credential store, not a token-helper-only mock: **3 behavioral REDs**. Downgraded/absent refresh scopes returned a prepared sender and overwrote encrypted credentials with stale capability metadata. Successful richer refresh also retained old scope aliases instead of recording actual scopes. The metadata-free legacy send-only path remains GREEN. A fixture syntax error was corrected before counting these behavioral REDs. Root approved `gmailProvider.ts` refresh-only scope. C3 explicitly released its committed serializer at `fea4eb94c1ec68200e2021ddc0f6ac7d5a21f1fb` before any C2 edit to that file. This report update remains uncommitted per root's latest coordination instruction.


I1 repair is committed **`417c3b90116e27ac481edf35e91c70ac86120fdd`**, exactly `src/main/outreach/providers/gmailProvider.ts` and `tests/main/googleGrantCredentials.test.ts`, using `git commit --only --` with resulting paths inspected. Source changes are limited to the capability helper import and refresh hunk, not the released serializer. Grant-bearing refresh requires explicit current scopes, rejects missing prior capabilities/unapproved powers, and persists actual returned scopes and derived capabilities rather than spreading stale grant claims. Metadata-free legacy send-only credentials retain their old compatibility path and acquire no invented metadata. The actual manager regressions prove downgraded/missing scopes yield `gmail_reauthorize`, no prepared sender, and a byte-for-byte unchanged old encrypted credential file. Successful rich refresh records the actual scope alias set. GmailProvider was released back to C3 after commit.

Final repair validation (all npm/npx shells used the mandatory Node24 PATH export):

- `npm test --prefix cloud/lambdas/delegated-worker -- test/remoteGoogleAuthorization.test.ts test/workerAuth.test.ts`: **36 passed** (25 OAuth, 11 auth/handler).
- `npx --no-install vitest run tests/main/googleGrantCredentials.test.ts tests/main/outreachProviders.test.ts tests/main/outreachProvidersOAuth.test.ts`: **33 passed** (10 C2 local, 17 provider, 6 OAuth).
- `npx --no-install vitest run tests/main/gmailThreadProvider.test.ts -t 'serializes real threaded send|rejects unsupported RFC ID|folds long ASCII References'`: **7 serializer regressions passed**, 10 unrelated tests intentionally excluded.
- Worker strict scoped compilation of `src/remoteGoogleAuthorization.ts test/remoteGoogleAuthorization.test.ts` and local scoped compilation of `src/main/outreach/providers/gmailProvider.ts tests/main/googleGrantCredentials.test.ts` passed. An initial local TypeScript optional-property inference error from spreading Zod output was fixed by preserving the preexisting typed credential spread while explicitly overriding validated grant metadata, then the local suite/typecheck/lint passed again.
- `npm run typecheck --prefix cloud/lambdas/delegated-worker` at 01:21:46 UTC: **exit 0** on the current assembled worker.
- Four-file scoped ESLint `--no-ignore ... --max-warnings 0`, owned diff checks and commit-path inspections passed. No full/native/package build run by C2.

A broader safety run of C3's currently changing `gmailThreadProvider.test.ts` observed 13 passing tests and four active foreign failures for unnamed attachment/message-rfc822/message-global evidence and HTML quoted-footer attribution. These exact failures were reported to C3, not edited by C2. The exact released serializer subset independently passed as noted above. Independent C2 rereview/root build and all prior activation/Terraform/live-service gates remain pending. Both Important findings have source/test repairs; no claim of independent approval is made. No cloud, real account, external provider, install, deployment or sender activation occurred.


## Second I2 repair: uncertain provider response

Read the full scoped rereview, including its distinction between lost Dynamo claim response and lost provider response. Root approved only the existing remote OAuth module/test. Commit **`bc16046be2089ca426880e94c117d73d4517d445`** changes exactly those two paths, using `git commit --only --`; resulting paths inspected.

**Root cause:** the first I2 repair still classified every request exception as a completed failure and persisted `revocationInFlight:false`. A local disconnect/timeout can occur while the remote provider continues processing. That incorrectly allowed a new revoke invocation to confirm cleanup and permit regrant.

**RED:** two new behavioral failures, for lost response/disconnect and timeout after submission. Simulated remote request A keeps processing independently after the local request throws. Invocation B incorrectly returned confirmed. This is separate from the retained lost-Dynamo-claim test.

**Repair:** on any exception without a definitive provider response, return pending without a finalizing write. The exact durable encrypted cleanup material and `revocationInFlight:true` are retained. B cannot submit another provider request, confirm cleanup or begin regrant, even when simulated remote work later finishes without an observed response. There is no automatic lease, timeout takeover or new reconciliation API. The operational uncertainty hold requires explicit reconciliation. Only a definite HTTP response (success or failure), or a no-provider-needed case, can release the claim according to status. The explicit-retry fixture was corrected to model a definitive HTTP 503 response rather than an ambiguous thrown network error.

**GREEN:** `npm test --prefix cloud/lambdas/delegated-worker -- test/remoteGoogleAuthorization.test.ts test/workerAuth.test.ts` passed **38 tests** (27 OAuth, 11 auth/handler). The scoped strict TypeScript command for the remote module/test and its imports passed with package compiler settings. Two-file scoped lint (`--no-ignore --max-warnings 0`) and diff checks passed. All npm/npx shells retained the mandated Node24 PATH. No full builds, external network/provider operations, C1 writes, new dependencies or changes outside the two authorized source/test paths. This report append remains uncommitted. Vole/root rereview is still required before C6 handler release.


## Final review closure and report cleanup

Vole's final `task-C2-review.md` section, “Final scoped C2 closure: bc16046”, approves scoped C2 source-review closure with no remaining findings. Root confirmed that closure at 2026-09-09 01:35 UTC, froze C2 sources and transferred handler ownership to C6. This supersedes the earlier historical rereview-pending and handler-release notes above. The source/test commit references and recorded RED/GREEN results were verified against the completed work for this report-only cleanup. Earlier statements that this appendix was uncommitted describe its checkpoint state; root explicitly authorized committing only this existing report at 02:13 UTC. No source changes or tests/builds were performed during cleanup.

Scoped review approval is not deployment or live-account acceptance. The ambiguous-revocation operational hold, Terraform provider-schema validation, final root integration/build gates and all activation/new-grant/outreach approvals remain separate.
