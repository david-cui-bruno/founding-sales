...
describe("PII-safe handler logging", () => {
  it("serializes only the package policy for PII-bearing inputs", () => {
    ...
    try {
      log("info", 'run finished', { written: 4, entitiesScanned: 6, ownerName: "Private Person", row: { email: "private@example.test" } });
    } finally {
      spy.mockRestore();
    }
    ...
    expect(record.component).toBe('adapter-boston-assessments');
