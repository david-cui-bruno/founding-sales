...
export type MailPollTarget = { pairingId: string; accountId: string; mailboxSubject: string; knownThreadIds: string[]; participantAddresses: string[]; since: string };
...
export function createMailPoller(input: { authorization: Pick<RemoteGoogleAuthorization, 'authorizedAccess'>;
  ...
  return { async pollOnce(target: MailPollTarget, signal: AbortSignal) {
    ...
    try {
      ...
      const cursor = await input.store.checkpoint(target.accountId, access.grant.subject);
      const provider = createGmailThreadProvider({ ...access, fetch: input.fetch });
      const page = await provider.readRelevantThreads({ accountId: target.accountId, knownThreadIds: target.knownThreadIds,
        participantAddresses: target.participantAddresses, since: target.since, cursor, maxPages: 1, maxBodyBytes: 12000 }, signal);
