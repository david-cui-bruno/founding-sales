...
import type { ResearchOnceRequest } from './researchOnceContract';
...
export type ResearchCoordinatorOptions = { auth: WorkerAuth; fetch: typeof globalThis.fetch; research?: SourceResearchBoundaries; researchSetupProfile?: ResearchSetupProfile };
...
export async function runResearch(input: ResearchCoordinatorOptions, signal: AbortSignal, report: SourceTickReport, once?: ResearchOnceRequest) {
    ...
    const runId = researchRunId(config.workspaceId, config.pairingId, settings);
    if (once) {
