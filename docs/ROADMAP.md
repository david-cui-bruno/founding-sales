...
- Separate saved draft continuations from saved reply history. Existing campaign versions remain read-only, without a fake Approve or Activate action.
...
The campaign-draft regression separately uses the real Campaigns route, delegation runtime, authenticated worker handler and local encrypted SQL projector, with the existing synthetic DynamoDB harness and fictional accounts. It covers owner-applied saving, a fresh database connection, and same-command reconciliation after route closure. It is not a deployed-worker, full preload, process-restart, phone or provider test. A renderer browser check covers the form, retained input, keyboard editing and theme/viewport accessibility, not a second simulated worker-save path.
...
| --- | --- | --- |
| 1 | Deliver a coherent candidate | The first eleven PRs are merged. Main `5dd96d5` passed hosted checks and a signed candidate gate with 31 packaged workflows. That candidate does not include subsequent campaign-draft changes. Installation and one real-company walkthrough still require a separate backed-up installation decision. Subsequent PRs need their own affected checks, not reruns of every historical checkpoint. |
...
| 3 | Make small campaigns usable | The bounded unapproved call-draft path uses the existing versioned owner-command backend, with a readable exact company audience and offer. Approval and enrollment are still missing, and real worker setup/ownership remain prerequisites. Qualify this small path before adding multi-step/email/LinkedIn campaign editing. No pretend activation. |
