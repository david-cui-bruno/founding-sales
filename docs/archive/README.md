...
- `prototypes/` (from `docs/prototypes/`): the 9 September Today layout review, the three compact Today studies (David chose A, Native Desk) and the visual research note. `prototypes/2026-09-09-today-studies.html` is still read by `tests/browser/nativeDesk.spec.ts` as the reference composition, and `prototypes/2026-09-09-today-studies.verify.mjs` defaults to its new path.
