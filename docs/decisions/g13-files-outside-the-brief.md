...
`docs/greenfield/install.md` and `docs/decisions/g13-*.md`. Deliverable 2 — "the app
...
| `apps/desktop/src/main/bundleScheme.ts` | `callie-app://`, because `file://` cannot read an asar with the fuse burned off. `docs/decisions/g13-bundle-scheme.md`. |
...
`docs/decisions/g13-keychain-acl.md`. G2's unit test asserts the argument vector and
