...
The second run, the same evening, reported seven more denials, again the check's and not the policy's: `ecs:CreateService`, `ecs:UpdateService`, `ecs:DeleteService`, `ecs:RunTask`, `ecs:DescribeTasks` and `ecs:ListTasks` simulated against the cluster ARN, and `elasticloadbalancing:CreateTargetGroup` against the load balancer's. The simulator answers implicit deny whenever the ARN is not of a type the action can be authorized on, whatever the policy says; the real calls are judged on the service, the task definition, the task, the container instance and the target group, every one of which `NamedResourcesInThisNamespace` names without a condition (the target group also under `elasticloadbalancing:Create*` with the request tag). The override table now carries those five resource types, and `cloudwatch:PutCompositeAlarm` is judged against `alarm:*` in every group that names it. The rule for whoever adds a group next: the sample ARN must be of the resource type the IAM action table gives for every action in the group, or the action gets a row in the override table.
