...
The lines from 21 to 25 September 2026 were seeded from the release records 8.0 to 8.0au, one line each, dated as the record dates itself. The bracketed number ending each names its record in [`release-records.md`](release-records.md). Changes merged in those days without a record of their own are not listed; `git log --merges` has them.
