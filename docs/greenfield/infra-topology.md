...
| CloudWatch custom metrics | Per metric-month | heartbeats, job age, canary age, watch expiry, mailbox state, held counts, plus the three log-derived safety metrics (four metric filters) | application behaviour | same |
| CloudWatch alarms | Per standard alarm-month; metric-math and composite alarms are billed differently | 16 standard alarms + 1 metric-math alarm + 2 composite alarms (the critical and warning roll-ups), none with an action | thresholds are variables in `infra/modules/alerts` | same |
