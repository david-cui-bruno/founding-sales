...
14. [every stage] **Nothing of the run is left, and nothing production's was touched** (`infra/scripts/rehearsal.sh guard <prefix>`), always, including on a run that created nothing. Three facts, since P7 (27 September 2026): `terraform state list` for the run's state key is empty, or the root was never initialised, so the teardown destroyed everything the run's state held and nothing in that state was production's; the session is an assumed-role session of `fss-rh-deploy`, whose policy is scoped to `fss-rh-*`; and one RDS describe of `<prefix>-pg`, the run's database and its most expensive resource, answers `DBInstanceNotFound`. **Why it no longer compares an inventory.** Until P7 this step read every resource tagged with the run prefix through the tagging API and compared it with item 3's record. That comparison had to learn every shape AWS keeps listing after it accepted a deletion (a stopped task, a deregistered task-definition revision, a Fargate task's network interface, a deleted security group and its rules, a key scheduled for deletion, a production database's retained automated backups; lanes G47, G52 and g97), and each lesson cost a failed run. It saw only the rehearsal's region besides (8.1, item 1). Until lane g97 it compared the production inventory, which failed whenever production was legitimately changed during a run. That nothing production's was touched still rests on what made it true before: no rehearsal command names production (refused at the call), the session, and the role's policy. Rehearsal databases delete their automated backups on teardown (`database_delete_automated_backups = true` in `infra/roots/rehearsal`; production keeps them).
...
1. **Answered by removal (P7, 27 September 2026).** The guard no longer reads the tagging API, whose answer was regional: it reads the run's Terraform state, the session and the run's database (3, item 14). What it cannot see is a resource the run made outside its own state, in any region; nothing in the rehearsal does.
