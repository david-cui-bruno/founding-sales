...
| `full` (default) | the declared ranges against the deployed images; and in `mode: full` the release suite, the drill's evidence and the restore drill, the journal replay and Gmail reconstruction, and the release record | in `mode: schema`, that the new schema and both images agree in a real environment; in `mode: full`, the release gate of 16.2, which is everything in the numbered list below | about 45 minutes (`schema`), up to the 180-minute timeout (`full`) |
