...
In the rehearsal, `infra/scripts/rehearsal-restore-drill.sh` keeps the AWS calls on the runner and runs the database work as two one-off tasks inside the VPC. The first is `fss admin counts --as-of` for the baseline, on the source, before the restore. The second is one `fss drill` against the restored instance for steps 1 to 9, which runs them in one process and writes one JSON report per step. **The baseline has to be handed to that drill task, and today it is not.** The counts task's answer comes back through its log stream to the runner, where the step 0 refusal reads it, and it stays there. The drill task is launched with `--as-of` rather than `--baseline`, so it measures step 0 again against the restored instance and writes it under `--reports /tmp/fss-drill`, a directory nothing in the container creates. The thirteenth full run stopped exactly there (`docs/greenfield/release.md` 8.0w). Until the baseline reaches the drill task, as a value in the task override or as an object the task can read, the rehearsal cannot run steps 1 to 9 against the baseline measured before the restore.
