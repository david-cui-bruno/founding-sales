...
Verified existing migration directory and `0022MailPersistence.ts`/`0023Campaigns.ts`; `0024RequestedPhoneFollowup.ts` does not yet exist and is a proposed Create path, not an implemented migration. Camel reserves the next migration number at root approval if another additive migration has already landed. Do not modify historical21/22/23. Reuse local immutable owner command/event storage for attestation/approval evidence. Remote drafts use `MAIL_REQUESTED_DRAFT#<encodedAccount>#<encodedDraft>` in the existing workspace partition. Reuse existing `DISPATCH_PERMISSION`, `DISPATCH_APPROVAL`, `DISPATCH_INTENT`, `COMMAND` and accepted-send evidence records with strict new variants. The one additional remote REQUESTED_APPROVAL continuation item is specified below. No extra consent ledger, mailbox table or synthetic `MAIL_THREAD` is needed.

### Durable one-click continuation (no stale command replay)

**Record:** one narrowly scoped remote `REQUESTED_APPROVAL#<encoded ownerCommandId>` item, not a workflow engine or new SQL table. Its strict record contains `{commandId,commandFingerprint,accountId,pairingId,mailboxSubject,authorityGeneration,draftSnapshot,requestSnapshot,originalCall,baselineMailContext,expiresAt,state,scopePlan,lastReason,materializedIntentId}`. `draftSnapshot` is the exact user-reviewed draft including content/recipient/context. `requestSnapshot` includes the authenticated attestation and principal. `scopePlan` is null or the server-derived immutable `{expectedEnvelopeRevision,previousScopeFingerprint,desiredScope}`. State is `pending_preflight | materialized | needs_review | expired | revoked`; only materialized may point to an executable intent. Transient provider/disconnection errors leave pending_preflight with a bounded reason, not a permanent rejected owner command.

**Initial capture, C6:** validate the original owner command's current version and receipt/request/draft, then atomically write the ordinary immutable `COMMAND#commandId` applied receipt, pending record, AUTH update and ordered `requested_followup.status` outbox event. Here applied means **submission captured**, not content executable or email sent. No permission/approval/intent/action is materialized yet. The new typed API returns `RequestedApprovalStatus {receipt:CommandReceipt,state,intentCommandId:string|null,reason:string|null}`. Its draft get response also exposes this status. Existing unrelated command receipts retain their meaning. Local replay retains status in the new draft table's nullable approval_json, separate from the strict draft DTO and linked to the owner command; no new local approval table.

**Resume owner, C6:** extend the actual `cloud/lambdas/delegated-worker/src/sourceCoordinator.ts` `submittedCommands` branch to recognize captured `approve-requested-followup` commands. Reuse its bounded25 COMMAND page, persisted scan cursor, deadlines, active-source/pairing checks and fairness. The branch loads the exact pending record and original applied receipt. Local sync only retransmits an unacknowledged identical command and consumes status events, never restarts preflight or creates a replacement approval command. The worker resumes with current AUTH/version CAS and the original approved generation/pairing/mailbox/account/call/content/expiry, not the initial now-stale expectedVersion. Only aggregate version refreshes. A changed generation or pairing is not rebased and moves to needs_review; expiry never extends. Pause/disconnection defers automatically; revoked authority, expired request or genuine changed evidence becomes terminal revoked/expired/needs_review.

**Scope/preflight:** before scope IO, CAS-persist one deterministic desired scope in scopePlan, derived from the captured owner request and current complete participant set. Run existing C3 admitScope/poller. If a crash occurs after scope commit, resume recognizes the exact stored desired scope fingerprint and does not increment/admit it again. A different concurrent scope is not silently overwritten. Incomplete pages, HTTP failures and crash-left pending attempts retain pending_preflight; a later source tick starts a new bounded attempt from the real checkpoint. Compare original account/call/recipient/content and actual retained-thread fingerprint throughout. Only the intentional scope-only revision transition may be rebound with identical mail evidence. Newly discovered relevant mail moves to needs_review. Missing scope, null semantic proof or incomplete/failed poll never materializes approval; suppression is never removed.

**Final materialization, C4/C1:** reread current pending/source/pairing/AUTH/account/recipient/receipt/draft/cursor/suppression/revocation. One existing Dynamo transaction CASes pending→materialized, writes the deterministic permission/approval/intent and prepared action, appends its ordered status outbox, and advances current AUTH/version without changing owner/generation. Each final key derives from captured commandId/approvalId/intentCommandId/actionId. Original COMMAND receipt remains immutable. C1 owner exports `planPrepareAction(input,currentAuthority): {items,preparedAction}` using the existing prepareAction input and actionIdentity/preparedSchema validation. It returns one conditional prepared-action Put when absent or an identical-action Check when already present, never two operations on the same Dynamo item, and no AUTH/outbox mutation. Existing prepareAction delegates to that same planner, not a second algorithm. C4 likewise exposes planned immutable permission/approval/intent items; the continuation must not call the existing sequential admitPermission/admitApproval/admitIntent writers. Root must serialize this export and its tests. Competing resumptions have one CAS winner. An ambiguous commit is reconciled by reading the pending record and exact immutable materialized keys before attempting any work again.

After materialization, the same source branch uses existing C4 dispatch for prepared/queued work and reconciliation for reserved/unknown work. It never infers send success from materialized or applied. Materialized is not sufficient authority to send: actual reserve still rechecks policy, grants, caps and all current fences. No synthetic submit command and no second routine human approval is needed. Source ticks can defer for existing activation constraints, but no permanent pending state results solely from scope-induced aggregate-version changes.
...
- [ ] Add the strict new variant. Implement the two exact transaction boundaries above: initial capture of the immutable submission, then resumable final materialization under current CAS after preflight. Partial capture/admission must not leave an executable action. Final reservation verifies both the applied capture command and its exact materialized continuation record. Replay must return the same receipt, not recreate permission after revocation.
...
**Proposed owner:** humpback/C6 after desktop freeze, shared command wiring serialized by root. **Files:** Modify `src/main/delegation/delegationRuntime.ts`, `src/main/ipc/registerOutreachIpc.ts`, `src/preload/createCallieApi.ts`, and `src/main/delegation/executionClient.ts` for the bounded owner-proof read; worker `cloud/lambdas/delegated-worker/src/sourceCoordinator.ts` for the one narrow continuation branch. Add cases in existing `cloud/lambdas/delegated-worker/test/sourceCoordinator.test.ts`. Contracts are in Task1's `src/shared/contracts/requestedFollowupContract.ts`. Test `tests/integration/delegatedWorkflow.test.ts` and create `tests/integration/requestedPhoneFollowup.test.ts`. All named backend files exist. No renderer/composer/layout edits are included before D3.
...
type RequestedApprovalStatus = {receipt:CommandReceipt;
  state:'pending_preflight'|'materialized'|'needs_review'|'expired'|'revoked';
  intentCommandId:string|null; reason:string|null};
...
approveRequestedFollowup(input:ApproveRequestedFollowup):Promise<RequestedApprovalStatus>;
```
...
- [ ] Prove GREEN initial applied-capture/non-executable status, crash before/after scope admission, incomplete multiple pages, HTTP failure then automatic source-tick recovery, scope-induced AUTH changes without stale-command resubmission, duplicate concurrent continuations, ambiguous final commit, source restart, original-content mismatch, new-mail needs_review, pause/revoke/expiry, and materialized-but-unknown send reconciliation. Assert exactly one initial human approval and no second human action for unchanged-evidence scope-only changes. Also verify restart/offline-pending/reconnect and retained edits. Repeat reply-only regression tests unchanged. Root owns packaged/normal activation acceptance separately.
