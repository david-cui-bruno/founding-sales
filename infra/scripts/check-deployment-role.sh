...
# The account and the region default to the one FSS runs in. Both are only ever used to
# build the *sample* ARNs the simulation is run against, so a wrong one shows up as a
# simulation of the wrong namespace rather than as a call against the wrong account.
#
...
alarms, including the two composites (denied 21 Sep), and the drill's read of one's history (g56)|cloudwatch:PutMetricAlarm,cloudwatch:PutCompositeAlarm,cloudwatch:DeleteAlarms,cloudwatch:DescribeAlarms,cloudwatch:TagResource,cloudwatch:DescribeAlarmHistory|arn:aws:cloudwatch:${REGION}:${ACCOUNT}:alarm:${PREFIX}-example-critical|
