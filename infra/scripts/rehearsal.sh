...
# FSS_REHEARSAL_JOURNAL_BUCKET, FSS_REHEARSAL_ROOT and lib.sh's FSS_RELEASE_* fixtures.
...
rehearsal_teardown() {
  ...
  rehearsal_write_report "teardown.txt" "prefix=$prefix destroyed=$destroyed journal_bucket=gone"
  rehearsal_log "torn down; rehearsal.sh guard $prefix proves nothing of the run was left and nothing production's was addressed"
}

# ---------------------------------------------------------------------------
# guard: Appendix G 39's cloud half, on every run, after the teardown (P7: it replaces the
# tagging-API comparison, which had to learn every shape AWS keeps listing after it
# accepted a deletion, lanes G47, G52 and g97). Three facts:
...
#   3. one RDS describe: `<prefix>-pg`, the run's database and its most expensive thing, is
#      gone (DBInstanceNotFound).
...
# ---------------------------------------------------------------------------
rehearsal_guard() {
  local prefix=${1:-} state state_read=true offending output status
  rehearsal_require_prefix "$prefix" || exit 1
  rehearsal_log "asserting the run left nothing in its state, touched nothing production's, and deleted its database"
  ...
  set +e
  output="$(rehearsal_aws rds describe-db-instances --db-instance-identifier "${prefix}-pg" \
    --query 'DBInstances[0].DBInstanceStatus' --output text 2>&1)"
  status=$?
  set -e
  if [ "$status" -eq 0 ]; then
    rehearsal_fail "the run's database ${prefix}-pg still exists after the teardown (${output:-no status}); delete it, it bills by the hour"
  fi
  case "$output" in
    *"(DBInstanceNotFound)"* | *"(DBInstanceNotFoundFault)"*) ;;
    *) printf '%s\n' "$output" >&2; rehearsal_fail "${prefix}-pg could not be described, and not because it is gone" ;;
  esac

  rehearsal_write_report "prefix-guard.txt" \
    "prefix=$prefix production_untouched=true state_empty=true database=absent state_read=$state_read"
  rehearsal_log "pass: the run's state is empty, its database is gone, and nothing production's was addressed"
}
