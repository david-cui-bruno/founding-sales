...
# checkout — a commit from before this lane has no `rollback.sh` — and uses the
...
#      again. Across that boundary only the images roll back, without Terraform (release.md
#      4.1a), and the infrastructure is repaired forward. The check is `git merge-base
#      --is-ancestor`; a checkout that does not hold the boundary commit cannot answer it
#      and is refused as well (fetch, then run this again).
...
#   5. **A restore in progress, unnamed** (W3-S8's restore runbook, PR 282). Between the
#      runbook's steps (f) and (g) production runs on a point-in-time copy:
#      `FSS_DATABASE_HOST` on the running API and worker is the copy's address, not the
#      managed instance's (the root's `database_endpoint` output), and every production
#      plan must carry `-var=active_database_host=<copy address>`, or it points production
#      back at the old instance. So when the two differ this refuses to plan unless
#      `--active-database-host` names the host production runs on, and then passes it
#      through. A host given that is not the running one is refused too: a rollback moves
#      images, never the database.
#   6. **A plan that is more than a rollback.** A rollback registers task definitions and
...
# carries production's settings. A value read wrong shows as a change to the listener, a subscription or a task
# definition's neighbour, which refusal 6 refuses. `bootstrap=false`, and the schema ranges
...
# (release_output's fixtures), FSS_REHEARSAL_REPORTS, the waits
# FSS_ROLLBACK_CANARY_ATTEMPTS/_SECONDS and FSS_ROLLBACK_SMOKE_ATTEMPTS/_SECONDS, and
# FSS_ROLLBACK_BOUNDARY_COMMIT (the boundary commit, for a test repository that cannot hold
# beed2d90). `test/ops/releaseRollback.check.ts` drives the whole script against stub
# `aws`, `terraform` and `curl` processes.
#
...
# PR 272's merge: no production root from it on declares the restore drill (refusal 0).
BOUNDARY_COMMIT="${FSS_ROLLBACK_BOUNDARY_COMMIT:-beed2d90e6fde2147a12b069aaf2f4d20ae4b8ad}"
...
usage() {
  echo "usage: rollback.sh <terraform root> <name prefix> --api-digest D --worker-digest D [--active-database-host H] [--apply]" >&2
  exit 1
}
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --active-database-host) [ "$#" -ge 2 ] || usage; ACTIVE_DATABASE_HOST=$2; shift 2 ;;
    --apply) APPLY=1; shift ;;
    *) rollback_fail "rollback.sh does not take '$1'" ;;
  esac
done
...
if [ -n "$ACTIVE_DATABASE_HOST" ] \
  && [[ ! "$ACTIVE_DATABASE_HOST" =~ ^[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?(\.[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?)+$ ]]; then
  rollback_fail "--active-database-host '$ACTIVE_DATABASE_HOST' is not a lower-case DNS hostname with at least one dot (no scheme, no port, no path), as the root's active_database_host requires"
fi
...
# 0. Not across the PR 272 boundary: an older commit's Terraform re-creates the drill.
if ! git -C "$CHECKOUT" cat-file -e "${BOUNDARY_COMMIT}^{commit}" 2>/dev/null; then
  rollback_fail "the checkout at $CHECKOUT does not hold ${BOUNDARY_COMMIT:0:8}, the merge of PR 272, so nothing can say whether $COMMIT is older than it: fetch origin there (git -C $CHECKOUT fetch origin) and run this again"
fi
...
if [ -n "$ACTIVE_DATABASE_HOST" ] && ! grep -qE '^variable "active_database_host" *\{' "$ROOT_DIRECTORY"/*.tf 2>/dev/null; then
  rollback_fail "$COMMIT's production root declares no active_database_host, so its plan cannot keep production on $ACTIVE_DATABASE_HOST; roll back to a commit that has it, or finish the restore first (runbooks/restore.md (g)). Nothing was planned."
fi
...
if rehearsal_dry_run; then
  ...
  # The database host is that reader's too (refusal 5): one place reads a running task
  # definition's environment, and it is the one that already refuses a revision nothing runs.
  API_DATABASE_HOST="$(printf '%s\n' "$DEPLOYED" | sed -n 's/^api_database_host=//p')"
  WORKER_DATABASE_HOST="$(printf '%s\n' "$DEPLOYED" | sed -n 's/^worker_database_host=//p')"
  [ -n "$API_DATABASE_HOST" ] && [ -n "$WORKER_DATABASE_HOST" ] \
    || rollback_fail "deploy.sh current reported no FSS_DATABASE_HOST for $PREFIX-api and $PREFIX-worker, so nothing can say which database production runs on"
...
if rehearsal_dry_run; then
  ...
  rehearsal_plan "read FSS_SENDING_ENABLED (both, which must agree) and the API's FSS_PUBLIC_ORIGIN"
  SENDING='<FSS_SENDING_ENABLED as it runs>'
  API_HOSTNAME='<the host of FSS_PUBLIC_ORIGIN>'
  LIVE_DATABASE_HOST='<FSS_DATABASE_HOST as deploy.sh current reports it>'
else
  for service in api worker; do
...
print(sending[0], match.group(1))
PY
)" || exit 1
  read -r SENDING API_HOSTNAME <<<"$FLAGS"
fi
...
rehearsal_log "sending_enabled=$SENDING, api_hostname=$API_HOSTNAME, as production runs them"
...
if [ -n "$ACTIVE_DATABASE_HOST" ]; then PLAN_VARIABLES+=(-var="active_database_host=$ACTIVE_DATABASE_HOST"); fi
