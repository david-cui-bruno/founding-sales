...
export function validatePreReleaseReceipt(value) {
  if (!value || JSON.stringify(Object.keys(value).sort()) !== JSON.stringify(['basename', 'createdAt', 'kind', 'schemaVersion', 'sha256', 'sizeBytes', 'verifiedAt'])
    || !/^pre_release-[0-9]{8}T[0-9]{9}Z\.sqlite3$/.test(value.basename) || value.kind !== 'pre_release' || value.schemaVersion !== 15
    || !/^[0-9a-f]{64}$/.test(value.sha256) || !Number.isSafeInteger(value.sizeBytes) || value.sizeBytes <= 0
