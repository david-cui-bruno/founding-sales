...
export function validatePreReleaseReceipt(value) {
  if (!value || JSON.stringify(Object.keys(value).sort()) !== JSON.stringify(['basename', 'createdAt', 'kind', 'schemaVersion', 'sha256', 'sizeBytes', 'verifiedAt'])
    || typeof value.basename !== 'string' || !/^pre_release-[0-9]{8}T[0-9]{9}Z\.sqlite3$/.test(value.basename) || value.kind !== 'pre_release' || (value.schemaVersion !== 24 && value.schemaVersion !== 25 && value.schemaVersion !== 26)
