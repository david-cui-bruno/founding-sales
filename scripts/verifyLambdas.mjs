...
export function discoverLambdaPackages({ root = projectRoot, run = spawnSync } = {}) {
  ...
  if (!manifests.includes('cloud/lambdas/shared/package.json')) throw new Error('LAMBDA_SHARED_MISSING');
  return manifests.sort((a, b) => a.includes('/shared/') ? -1 : b.includes('/shared/') ? 1 : a.localeCompare(b)).map(path => {
    ...
    const phases = path.includes('/shared/') ? ['typecheck', 'test'] : ['typecheck', 'test', 'build'];
