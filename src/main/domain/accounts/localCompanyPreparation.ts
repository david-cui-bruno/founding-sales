...
type ResearchRow = { completed: number; pending: number; sources: number };
...
export function readLocalCompanyPreparation(database: AppDatabase, snapshot: AccountEvidenceSnapshot): LocalAccountPreparation {
  try {
    ...
    const research = database.raw.prepare(`SELECT
      ...
      EXISTS(SELECT 1 FROM pm_account_sources WHERE account_id=? AND permitted=1) AS sources`).get(accountId, accountId, accountId) as ResearchRow;
