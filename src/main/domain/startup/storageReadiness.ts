...
export type DomainStorageReadiness<Version extends 24 | 25 | 26 | 27 | 28 | 29 | 30 = 30> = Readonly<{
...
export const SCHEMA26_MANIFEST: DomainSchemaManifest = Object.freeze({
...
// Schema 27 rebuilds pm_account_routes so verification may be 'listed'; every object name is
// unchanged, only that table's CREATE statement (and therefore the SQL fingerprint) differs.
...
export const SCHEMA29_MANIFEST: DomainSchemaManifest = Object.freeze({
...
// Schema 30 adds the follow-up template table, the singleton pause switch and their four triggers; no index, nothing else changes.
export const DOMAIN_SCHEMA_MANIFEST: DomainSchemaManifest = Object.freeze({
  tables: Object.freeze([...SCHEMA29_MANIFEST.tables, 'email_templates', 'email_template_settings'].sort()),
  indexes: SCHEMA29_MANIFEST.indexes,
  triggers: Object.freeze([...SCHEMA29_MANIFEST.triggers, 'email_templates_no_delete', 'email_templates_revision',
    'email_template_settings_no_delete', 'email_template_settings_revision'].sort()),
  catalogSha256: '5e1b6a528a6e7e4378ace0bbad37d8adc6eb0f9a28ff25c87c697d891b9e1aed',
});
...
export function assertPreReleaseStorageReady(database: AppDatabase): DomainStorageReadiness<24 | 25 | 26 | 27 | 28 | 29 | 30> {
  const row = appMetaSchema.safeParse(database.raw.prepare('SELECT schema_version FROM app_meta WHERE singleton = 1').get());
  if (!row.success || (row.data.schema_version !== 24 && row.data.schema_version !== 25 && row.data.schema_version !== 26 && row.data.schema_version !== 27 && row.data.schema_version !== 28 && row.data.schema_version !== 29 && row.data.schema_version !== 30)) {
  ...
  const manifest = version === 24 ? SCHEMA24_MANIFEST : version === 25 ? SCHEMA25_MANIFEST : version === 26 ? SCHEMA26_MANIFEST : version === 27 ? SCHEMA27_MANIFEST : version === 28 ? SCHEMA28_MANIFEST : version === 29 ? SCHEMA29_MANIFEST : DOMAIN_SCHEMA_MANIFEST;
...
function assertExactStorageReady<Version extends 24 | 25 | 26 | 27 | 28 | 29 | 30>(input: {
  database: AppDatabase;
  expectedBusyTimeoutMs: 5000;
