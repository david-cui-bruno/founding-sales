import { localKnownCompanyConfigurationSchema, type LocalKnownCompanyConfiguration } from '../../shared/contracts/localCompanyResearchSettingsContract';
import { validateKnownCompanyActivation } from './knownCompanyRequestProfile';
import { z } from 'zod';
...
export type EffectiveCompanyResearchConfiguration = CompanyResearchStartupConfiguration | LocalKnownCompanyConfiguration;
export function isLocalKnownCompanyConfiguration(config: EffectiveCompanyResearchConfiguration): config is LocalKnownCompanyConfiguration { return 'mode' in config && config.mode === 'known_company'; }
...
export function validateCompanyResearchConfiguration(config: EffectiveCompanyResearchConfiguration): void {
  if (isLocalKnownCompanyConfiguration(config)) {
    const parsed = localKnownCompanyConfigurationSchema.parse(config);
    if (parsed.state !== 'active') throw new Error('Research configuration paused');
    validateKnownCompanyActivation(parsed);
  } else discoveryInputFingerprint(config);
