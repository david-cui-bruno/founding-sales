...
it('never offers LinkedIn routes for the call template', () => {
  const f = fixture(true);
  const snapshot = structuredClone(f.props.snapshot);
  snapshot.accounts[0].routes = [linkedIn, { ...linkedIn, id: 'thread', value: 'https://www.linkedin.com/messaging/thread/fictional-thread' }, phone];
  render(<CallCampaignEnrollment {...f.props} snapshot={snapshot} />);
  expect(screen.getByRole('region', { name: 'Call campaign enrollment' })).toBeTruthy();
  expect(screen.queryByLabelText('Business LinkedIn route')).toBeNull();
  expect([...screen.getByLabelText<HTMLSelectElement>('Business phone route').options].map(option => option.value)).toEqual(['', 'phone']);
  expect(f.submit).not.toHaveBeenCalled();
});
