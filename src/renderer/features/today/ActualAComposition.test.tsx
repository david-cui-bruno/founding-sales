...
it.each([
  ...
  render(<NativeDesk onOpenImport={(): void => undefined} firstUse={f.firstUse} snapshot={snapshot} api={f.api} config={configuredFixtureStatus()} onRefresh={vi.fn()} onOpenLead={vi.fn()} localRead={mode === 'missing' ? undefined : {overview:{value:localSnapshot(),pending:false,error:false},retained}} />);
...
for (const size of [0, 1] as const) for (const mode of ['known', 'partial', 'read_failed', 'held_partial', 'excluded_foreign', 'unavailable'] as const) {
  it(`projects ${size} worker rows from their own ${mode} source on all three surfaces`, () => {
    ...
    const view = render(<NativeDesk onOpenImport={(): void => undefined} firstUse={f.firstUse} {...props} surface="today" />);
...
it.each([false, true])('shows held and applicable incomplete warnings independently with missing worker scope %s', missing => {
  ...
  const f = nativeDeskFixture(snapshot);
  render(<NativeDesk onOpenImport={(): void => undefined} firstUse={f.firstUse} snapshot={snapshot} api={f.api} config={null} localHold onRefresh={vi.fn()} onOpenLead={vi.fn()} />);
