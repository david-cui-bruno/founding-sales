...
it.each([
  ...
  render(<PresentationRoot><NativeDeskRoute firstUse={f.firstUse} api={f.api} surface={surface} onOpenLead={vi.fn()} /></PresentationRoot>);
...
it('keeps refresh accessible and icon-only beside persistent truthful worker status', async () => {
  ...
  render(<PresentationRoot><NativeDeskRoute firstUse={f.firstUse} api={f.api} onOpenLead={vi.fn()} /></PresentationRoot>);
...
it.each(['today', 'accounts', 'campaigns'] as const)('marks unresolved and first-failed %s as presentation only', async surface => {
  ...
  const view = render(<PresentationRoot><NativeDeskRoute firstUse={f.firstUse} api={f.api} surface={surface} onOpenLead={vi.fn()} /></PresentationRoot>);
...
it.each(['today', 'accounts', 'campaigns'] as const)('retains common presentation when actual %s legacy is confirmed', async surface => {
  ...
  const view = render(<PresentationRoot><NativeDeskRoute firstUse={f.firstUse} api={f.api} surface={surface} legacy={<p>Actual legacy Today</p>} onOpenLead={vi.fn()} /></PresentationRoot>);
