...
/** accountRepository.companyDraftPublication refuses such excerpts outright, so nothing can be offered from them. */
const unpublishable = /[<>]|\b(?:https?:|mailto:|javascript:)|\b(?:script|href|src)\s*=/i;
...
function matchedWord(text: string): string | null { return exclusionWord.exec(text)?.[0] ?? null; }
...
function lineTable(text: string): Line[] {
  const lines: Line[] = [];
  for (let start = 0; ;) {
...
/**
 * The blank-line-delimited paragraph around the address when it spans more than one line; otherwise the address line with
 * up to two non-blank lines before and one after, exactly as saved. Always a trimmed verbatim slice containing the occurrence,
 * so it passes the same occurrence check the domain applies before publication.
 */
function passage(text: string, lines: Line[], token: Span): string {
  ...
  if (first === last) {
    for (let count = 0; first > 0 && count < 2;) { first--; if (!lines[first].blank) count++; }
    for (let count = 0; last < lines.length - 1 && count < 1;) { last++; if (!lines[last].blank) count++; }
  }
