 ...
 * never mints a person from a new source. Compliance lives in the schema as
 * explicit evidence, and emails must already be lowercased (the suppression
 * HMACs assume it). Mirrors the cloud enrichmentPayloadSchema exactly.
 */
export const cloudContactComplianceEvidenceSchema = z
  .object({
...
export const cloudEnrichmentPhoneSchema = z
  .object({
    ...
    compliance: cloudContactComplianceEvidenceSchema,
