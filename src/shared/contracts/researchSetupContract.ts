...
export const researchSetupApproveInputSchema = z.strictObject({ expectedRevision: z.literal(0), descriptorFingerprint: hash, audience: audienceQuerySchema, permittedSources: z.array(z.url().max(2048)).max(500), maxCompanies: researchLimitsSchema.shape.maxCompanies, maxPages: researchLimitsSchema.shape.maxPages, maxBytes: researchLimitsSchema.shape.maxBytes, discoveryCeilingMicros: integer.positive(), researchCeilingMicros: integer.positive(), disclosureAcknowledged: z.literal(true), discoveryProvider: discoveryProviderSchema.optional() })
