...
const receipt = { path: '/must-not-print/private.sqlite3', basename: 'pre_release-20260906T170000000Z.sqlite3', kind: 'pre_release', schemaVersion: 16, sha256: 'a'.repeat(64), sizeBytes: 1234, createdAt: '2026-09-06T17:00:00.000Z', verifiedAt: '2026-09-06T17:00:00.000Z' };
...
it('holds the real host lock protocol through drain/close/zero, returning only allowlisted receipt', async () => {
  ...
  expect(key.bytes).toEqual(Buffer.alloc(32)); expect(result).toEqual({ basename: receipt.basename, kind: 'pre_release', schemaVersion: 16, sha256: 'a'.repeat(64), sizeBytes: 1234, createdAt: receipt.createdAt, verifiedAt: receipt.verifiedAt });
});
it.each([
...
async function workspace(version = 16) {
  ...
  try {
    const migrate = version === 16 ? migrateToLatest : createMigrationRunner(productionMigrations.slice(0, version));
    ...
    expect(database.raw.prepare('SELECT name FROM kysely_migration ORDER BY timestamp, name').all()).toHaveLength(version);
...
it('backs up genuine production16 with one verified copy and same-database linked receipt without migrations or timers', async () => {
  ...
  expect(result.kind).toBe('pre_release'); expect(result.schemaVersion).toBe(16); expect(JSON.stringify(result)).not.toContain(paths.databasePath);
  ...
  try {
    ...
    expect(copy.prepare('SELECT schema_version FROM app_meta').get()).toEqual({ schema_version: 16 });
  ...
  try {
    expect(db.raw.prepare('SELECT id, backup_basename, kind, schema_version, sha256, size_bytes, created_at, verified_at FROM backup_receipts').all()).toEqual([
      { id: 'fixture-receipt', backup_basename: result.basename, kind: 'pre_release', schema_version: 16, sha256: result.sha256, size_bytes: result.sizeBytes, created_at: result.createdAt, verified_at: result.verifiedAt },
    ]);
    expect(db.raw.prepare('SELECT schema_version FROM app_meta').get()).toEqual({ schema_version: 16 });
  } finally { closeDatabase(db); key.bytes.fill(0); }
  expect(loaded.every(k => k.bytes.equals(Buffer.alloc(32)))).toBe(true);
});
it.each(['missing', 'wrong-key', 'schema14', 'schema15', 'future17', 'schema15-marker16', 'schema15-marker-ledger16',
  ...
  const version = failure === 'schema14' ? 14 : failure.startsWith('schema15') ? 15 : 16;
  const { paths, key } = await workspace(version); const loaded = []; let unlocks = 0;
  try {
    const db = openDatabase({ path: paths.databasePath, key });
    try {
      if (failure === 'future17') db.raw.exec('UPDATE app_meta SET schema_version = 17'); // Unknown future marker, not a fictitious migration17.
      ...
      if (failure === 'ledger-missing') db.raw.exec("DELETE FROM kysely_migration WHERE name = '0016ContactPresentationEvidence'");
      if (failure === 'ledger-extra') db.raw.exec("INSERT INTO kysely_migration VALUES ('0017Unknown', '2099-01-01T00:00:00.000Z')");
      if (failure === 'ledger-wrong') db.raw.exec("UPDATE kysely_migration SET name = '0016Wrong' WHERE name = '0016ContactPresentationEvidence'");
      if (failure === 'ledger-order') db.raw.exec("UPDATE kysely_migration SET timestamp = '1900-01-01T00:00:00.000Z' WHERE name = '0016ContactPresentationEvidence'");
