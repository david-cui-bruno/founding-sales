...
const receipt = { path: '/must-not-print/private.sqlite3', basename: 'pre_release-20260906T170000000Z.sqlite3', kind: 'pre_release', schemaVersion: 22, sha256: 'a'.repeat(64), sizeBytes: 1234, createdAt: '2026-09-06T17:00:00.000Z', verifiedAt: '2026-09-06T17:00:00.000Z' };
...
it('holds the real host lock protocol through drain/close/zero, returning only allowlisted receipt', async () => {
  ...
  expect(key.bytes).toEqual(Buffer.alloc(32)); expect(result).toEqual({ basename: receipt.basename, kind: 'pre_release', schemaVersion: 22, sha256: 'a'.repeat(64), sizeBytes: 1234, createdAt: receipt.createdAt, verifiedAt: receipt.verifiedAt });
});
it.each([
  ['schemaVersion', 14], ['schemaVersion', 15], ['schemaVersion', 16], ['schemaVersion', 17], ['schemaVersion', 18], ['schemaVersion', 19], ['schemaVersion', 20], ['schemaVersion', 21], ['schemaVersion', 23], ['schemaVersion', '19'], ['schemaVersion', '20'], ['schemaVersion', '22'], ['schemaVersion', null], ['schemaVersion', true],
...
async function workspace(version = 22) {
  ...
  try {
    const registry = version === 23 ? [...productionMigrations, { id: '0023SyntheticFuture', schemaVersion: 23,
      migration: { async up(db) { await sql.raw('CREATE TABLE synthetic_future23 (id TEXT PRIMARY KEY)').execute(db);
        await sql.raw('UPDATE app_meta SET schema_version = 23').execute(db); } } }] : productionMigrations.filter(x => x.schemaVersion <= version);
    const migrate = version === 22 ? migrateToLatest : createMigrationRunner(registry);
    ...
    expect(database.raw.prepare("SELECT name FROM sqlite_master WHERE name = 'synthetic_future23'").get() !== undefined).toBe(version === 23);
...
it('backs up genuine production21 with one verified copy and same-database linked receipt without migrations or timers', async () => {
  ...
  expect(result.kind).toBe('pre_release'); expect(result.schemaVersion).toBe(22); expect(JSON.stringify(result)).not.toContain(paths.databasePath);
  ...
  try {
    ...
    expect(copy.prepare('SELECT schema_version FROM app_meta').get()).toEqual({ schema_version: 22 });
  ...
  try {
    expect(db.raw.prepare('SELECT id, backup_basename, kind, schema_version, sha256, size_bytes, created_at, verified_at FROM backup_receipts').all()).toEqual([
      { id: 'fixture-receipt', backup_basename: result.basename, kind: 'pre_release', schema_version: 22, sha256: result.sha256, size_bytes: result.sizeBytes, created_at: result.createdAt, verified_at: result.verifiedAt },
    ]);
...
it.each(['missing', 'wrong-key', 'schema14', 'schema15', 'schema16', 'schema17', 'schema18', 'schema19', 'schema20', 'schema21', 'future23', 'schema17-marker19', 'schema17-marker-ledger19', 'schema16-marker17', 'schema16-marker-ledger17', 'schema15-marker16', 'schema15-marker-ledger16',
  ...
  const version = failure === 'schema14' ? 14 : failure.startsWith('schema15') ? 15 : failure.startsWith('schema16') ? 16 : failure.startsWith('schema17') ? 17 : failure === 'schema18' ? 18 : failure === 'schema19' ? 19 : failure === 'schema20' ? 20 : failure === 'schema21' ? 21 : failure === 'future23' ? 23 : 22;
