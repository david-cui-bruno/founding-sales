...
it.each([
  ['schemaVersion', 14], ['schemaVersion', 15], ['schemaVersion', 16], ['schemaVersion', 17], ['schemaVersion', 18], ['schemaVersion', 19], ['schemaVersion', 20], ['schemaVersion', 21], ['schemaVersion', 22], ['schemaVersion', 23], ['schemaVersion', 30], ['schemaVersion', '19'], ['schemaVersion', '20'], ['schemaVersion', '22'], ['schemaVersion', null], ['schemaVersion', true],
...
async function workspace(version = 24) {
  ...
  try {
    const registry = version === 30 ? [...productionMigrations, { id: '0030SyntheticFuture', schemaVersion: 30,
      migration: { async up(db) { await sql.raw('CREATE TABLE synthetic_future30 (id TEXT PRIMARY KEY)').execute(db);
        await sql.raw('UPDATE app_meta SET schema_version = 30').execute(db); } } }] : productionMigrations.filter(x => x.schemaVersion <= version);
    ...
    expect(database.raw.prepare("SELECT name FROM sqlite_master WHERE name = 'synthetic_future30'").get() !== undefined).toBe(version === 30);
...
it.each([24, 25, 26, 27, 28, 29])('backs up genuine production%s with one verified copy and same-database linked receipt without migrations or timers', async version => {
...
it.each(['missing', 'wrong-key', 'schema14', 'schema15', 'schema16', 'schema17', 'schema18', 'schema19', 'schema20', 'schema21', 'schema22', 'schema23', 'future30', 'schema17-marker19', 'schema17-marker-ledger19', 'schema16-marker17', 'schema16-marker-ledger17', 'schema15-marker16', 'schema15-marker-ledger16',
  ...
  const version = failure === 'schema14' ? 14 : failure.startsWith('schema15') ? 15 : failure.startsWith('schema16') ? 16 : failure.startsWith('schema17') ? 17 : failure === 'schema18' ? 18 : failure === 'schema19' ? 19 : failure === 'schema20' ? 20 : failure === 'schema21' ? 21 : failure === 'schema22' ? 22 : failure === 'schema23' ? 23 : failure === 'future30' ? 30 : 24;
