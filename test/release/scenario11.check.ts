 ...
 * attempting the drill. What this file adds beyond that is the drill script run
 * offline: its refusals, its plan and its launches.
 *
 * ## The vacuous-pass trap
 *
 * A drill run against an empty restored database reconstructs nothing, reports zero
 * unresolved exceptions and passes in about four minutes. That is closed by
 * `rehearsal-restore-drill.sh` refusing to report a pass when the baseline counts are
 * zero, and the script is run here against an empty baseline to prove it.
 */
