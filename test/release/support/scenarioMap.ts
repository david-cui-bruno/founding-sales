...
export const SCENARIOS: readonly Scenario[] = Object.freeze([
  ...
  },
  {
    ...
    coverage: 'release',
    references: laneTest('packages/domain/db/migrations/0001_foundation.sql'),
    trap: 'A retention suite that only deletes proves nothing about what must survive.',
    closedBy: 'The check asserts every table is classified for retention, so a new table cannot be silently uncovered, and names the tombstones that must never be a deletion target.',
  },
  {
    number: 42,
