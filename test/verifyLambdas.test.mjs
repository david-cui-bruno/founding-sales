...
const names = ['adapter-boston-assessments', 'adapter-boston-rentsmart', 'adapter-pvd-taxroll', 'enricher', 'mail-parse', 'resolver', 'schedule-watchdog', 'scorer', 'suppression-sync', 'new-package'];
function fixture({ fail = '', missing = false } = {}) {
  ...
  for (const name of names) {
    ...
    writeFileSync(join(root, 'cloud/lambdas', name, 'package.json'), JSON.stringify({ scripts: { typecheck: 'tsc', test: 'vitest', ...(missing && name === 'resolver' ? {} : { build: 'node build.mjs' }) } }));
  }
...
it('discovers every tracked package plus additions in name order and builds each one', () => {
  const { root, run } = fixture(); expect(run().status).toBe(0);
  const calls = readFileSync(join(root, 'calls'), 'utf8').trim().split('\n').map(JSON.parse);
  expect(calls).toEqual([...names].sort().flatMap(name => ['typecheck', 'test', 'build'].map(phase => ({ cwd: join(root, 'cloud/lambdas', name), args: ['run', phase] }))));
});
