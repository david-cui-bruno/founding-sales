...
// The inventories before the legacy routes were removed (17 September 2026) and before the Apple spike was removed
// (18 September 2026) stay in Git history. Both are decoded from the pinned bytes rather than written here, so the
// scanned working tree never carries a second copy of either.
const decode = encoded => Buffer.from(encoded.match(/\\x([0-9a-f]{2})/g).map(byte => parseInt(byte.slice(2), 16))).toString();
const pinnedRetired = config.split(inventoryMarker)[1].match(/regexes = \['''[^']*''', '''\\A\\n\?((?:\\x[0-9a-f]{2})+)\\z''', '''\\A\\n\?((?:\\x[0-9a-f]{2})+)\\z'''\]/);
const retiredInventoryLines = [decode(pinnedRetired[1]), decode(pinnedRetired[2])];
...
it('limits the public API-name exception to the exact 138-byte current and 152-byte and 147-byte retired inventories, path and rule', () => {
  ...
  expect(Buffer.byteLength(line)).toBe(138);
  expect(sha256(line)).toBe('3a9901a7d0b3732e46b780a0c7c8b420dfd898c8c7e92398f1bad0ab1fb7bf91');
  const [retiredAppleSpikeLine, retiredLegacyLine] = retiredInventoryLines;
  ...
  expect(config.split(inventoryMarker)[1]).toBe(`, not a credential or a file waiver. Git history is scanned in full, so the lines retired on 17 and 18 September 2026 stay pinned beside the current one.\n[[allowlists]]\ndescription = "Exact public preload namespace inventory only: the current line and the lines retired on 17 and 18 September 2026"\ntargetRules = ["linkedin-client-id"]\ncondition = "AND"\npaths = ['''\\Atests/integration/appleSpikePreload\\.test\\.ts\\z''']\nregexTarget = "line"\nregexes = ['''\\A\\n?${encode(line)}\\z''', '''\\A\\n?${encode(retiredAppleSpikeLine)}\\z''', '''\\A\\n?${encode(retiredLegacyLine)}\\z''']\n`);
});
