...
test('P2 same executable migrates nonzero16 to19, retains exact encrypted backup and intentional action changes', async () => {
  ...
  try {
    ...
    expect(await workspace.page.evaluate(() => window.callie.health.get())).toMatchObject({ schemaVersion: 27, databaseEncrypted: true, domainReady: true });
    ...
    const migrated = await fixtures.inspectStoppedProfile('through16', material, 27);
    expect(migrated.preservedActionIndependentSha256).toBe(original.preservedActionIndependentSha256);
    expect(migrated.ledger.slice(-11)).toEqual(['0017DiscoveryAssessments', '0018PlaybookDueActions', '0019EmailDrafts', '0020PmAccounts', '0021DelegatedWork', '0022MailPersistence', '0023Campaigns', '0024RequestedFollowupAndPolicyReviews', '0025KnownCompanyResearchSettings', '0026LocalCompanyDrafts', '0027ListedRouteVerification']);
    workspace = await launch();
    ...
    expect((await fixtures.inspectStoppedProfile('through16', material, 27)).preservedActionIndependentSha256).toBe(original.preservedActionIndependentSha256);
