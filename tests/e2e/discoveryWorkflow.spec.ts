...
test('P2 same executable migrates nonzero16 to19, retains exact encrypted backup and intentional action changes', async () => {
  ...
  try {
    ...
    expect(await workspace.page.evaluate(() => window.callie.health.get())).toMatchObject({ schemaVersion: 21, databaseEncrypted: true, domainReady: true });
    ...
    const migrated = await fixtures.inspectStoppedProfile('through16', material, 21);
    expect(migrated.preservedActionIndependentSha256).toBe(original.preservedActionIndependentSha256);
    expect(migrated.ledger.slice(-6)).toEqual(['0016ContactPresentationEvidence', '0017DiscoveryAssessments', '0018PlaybookDueActions', '0019EmailDrafts', '0020PmAccounts', '0021DelegatedWork']);
    workspace = await launch();
    ...
    expect((await fixtures.inspectStoppedProfile('through16', material, 21)).preservedActionIndependentSha256).toBe(original.preservedActionIndependentSha256);
