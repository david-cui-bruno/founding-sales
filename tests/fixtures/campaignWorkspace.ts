...
/**
 * `companyLinkedInTarget` additionally admits one published company-level (personId null)
 * business LinkedIn route in the same evidence admission, so the account version stays 2.
 */
export async function createCampaignFixture(options: { companyLinkedInTarget?: string } = {}) {
  ...
  try {
    ...
    const linkedInRoute = options.companyLinkedInTarget === undefined ? null : { id: randomUUID(), accountId: account.id, personId: null as string | null,
      channel: 'linkedin' as const, value: options.companyLinkedInTarget, purpose: 'business' as const, evidenceIds: [sourceId], verification: 'published' as const };
    accounts.admitEvidence({ commandId: randomUUID(), accountId: account.id, expectedVersion: 1, sources: [{ id: sourceId, url: 'https://example.invalid/team', fetchedAt: now, sha256: 'a'.repeat(64), excerpt: 'Fictional business team contact.', permitted: true }], claims: [],
      routes: linkedInRoute ? [...routes, linkedInRoute] : routes });
    ...
    return { db, repo, account, routes, linkedInRoute, versions, workspaceId, now, path: temp.path, key, clock,
