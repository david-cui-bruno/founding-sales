...
export function requestedFollowupFixture(accountId = 'a1', accountVersion = 1, replyText?: string, outcome: 'connected' | 'voicemail' | 'no_answer' | 'busy' | 'interested' | 'gatekeeper' = 'connected') {
  ...
  const command = ownerCommandSchema.parse({ commandId, workspaceId: 'ws', accountId, expectedAuthorityGeneration: 0, expectedVersion: 1, kind: 'complete-manual',
    payload: { handoffId: handoff.handoffId, targetHash: handoff.targetHash, outcome: { actionId: handoff.actionId, channel: 'call', outcome, observedAt: REQUESTED_NOW, evidenceRef: 'owner-call-note', ...(replyText === undefined ? {} : { replyText }) } } });
  ...
  const evidence = stepEvidenceSchema.parse({ enrollmentId: 'enroll1', accountId, campaignVersionId: version.id, stepId: 'call-step', routeId: 'phone1', routeVersion: 1, outcome, observedAt: REQUESTED_NOW, observation: 'unknown', source: 'human', executionContextId: hash, contextRevision: 1, state: 'human_reported_sent', actionId: 'call1', channel: 'call' });
