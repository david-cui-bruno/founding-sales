...
it('prepares a durable draft from actual company-page extraction and B1 admission without a seeded LinkedIn route', async () => {
  ...
  try {
    ...
    const draft = await service.prepare({ stepId: version.steps[0]!.id, expectedVersion: 1 });
    ...
    expect((await new LinkedInService({ repository }).prepare({ stepId: version.steps[0]!.id, expectedVersion: 1 })).body).toBe('Human company-scoped edit');
  } finally { f.close(); }
});
