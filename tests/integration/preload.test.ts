...
describe('preload workflow bridge', () => {
  ...
  it('exposes exactly the composed workflow APIs plus the Apple spike surface', () => {
    ...
    expect(Object.keys(api.delegation).sort()).toEqual([
      ...
      'editRequestedFollowup', 'getPhoneHandoffState', 'getRequestedFollowup', 'googleConnections', 'pair', 'policyImport', 'prepareRequestedFollowup', 'researchSetup', 'status', 'submit', 'sync',
    ]);
    ...
    expect(Object.keys(api.localWorkspace).sort()).toEqual([
      ...
      'getCompanyResearchSettings', 'getCompanyResearchStatus', 'linkCompanyPerson', 'researchCompany', 'reviewCompany', 'transition', 'updateCallSettings', 'updateCompanyResearchSettings',
    ]);
