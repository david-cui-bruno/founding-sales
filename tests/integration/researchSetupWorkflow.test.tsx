...
import type { ResearchCycleAdmission, ResearchCycleStatusRequest } from '../../cloud/lambdas/delegated-worker/src/researchCycleContract';
...
it('reopens a separately admitted cycle, explicitly resumes via Settings, and retains both uncertain v1 histories after v2 completion', async () => {
  const f = await fixture({ now: new Date().toISOString(), discoveryCostMicros: 1000000, researchReservationMicros: 10000 });
  ...
  try {
    ...
    fireEvent.click(region.getByRole('button', { name: 'Resume research' })); await region.findByText(/Research policy request applied/);
    expect(ownerResearchSourceSchema.parse(f.dynamo.inspect(ownerResearchSourceKey()))).toMatchObject({ revision: 3, state: 'active', research: source.research });
    await f.unmount(); const second = await native(successor);
    expect(second).toMatchObject({ state: 'uncertain', settled: false });
    expect(discovery).toHaveBeenCalledTimes(2); expect(pageHttp).not.toHaveBeenCalled();
    f.mount(); region = within(await screen.findByRole('region', { name: 'Cloud research' }));
    await region.findByText('Existing policy (read-only): active');
    fireEvent.click(region.getByLabelText('I have reviewed the targeting, cumulative ceilings, operator assertions and limitations above'));
    fireEvent.click(region.getByRole('button', { name: 'Pause research' })); await region.findByText(/Research policy request applied/);
    await f.unmount();
    const paused = ownerResearchSourceSchema.parse(f.dynamo.inspect(ownerResearchSourceKey()));
    expect(paused).toMatchObject({ state: 'paused', revision: 4, research: source.research });
    const historicalKeys = [`DISCOVERY#${original.runId}`, `DISCOVERY#${second.runId}`, `RESEARCH_ONCE_NEXT#${original.runId}`];
    const historicalRows = historicalKeys.map(key => f.dynamo.inspect(key));
    const statusRequest: ResearchCycleStatusRequest = { version: 2, kind: 'research.cycle.status', workspaceId: request.workspaceId, pairingId: request.pairingId };
    const status = await native(statusRequest);
    expect(status).toMatchObject({ kind: 'research.cycle.status.result', source: { state: 'paused', revision: 4 }, discoveryBudget: { limit: 2000000, spent: 2000000 }, researchBudget: { limit: 10000, spent: 0 } });
    if (status.kind !== 'research.cycle.status.result' || !status.source?.researchFingerprint || !status.descriptorFingerprint
      || !status.predecessor || !status.observationFingerprint || !status.discoveryBudget || !status.researchBudget) throw Error('Expected coherent cycle admission observation');
    const proposal: ResearchCycleAdmission = { version: 2, kind: 'research.cycle.admit', workspaceId: request.workspaceId, pairingId: request.pairingId,
      expectedSourceRevision: status.source.revision, researchFingerprint: status.source.researchFingerprint, descriptorFingerprint: status.descriptorFingerprint,
      predecessor: status.predecessor, observationFingerprint: status.observationFingerprint, expectedDiscoveryBudget: status.discoveryBudget,
      expectedResearchBudget: status.researchBudget, proposedDiscoveryLimitMicros: 3000000, disclosureAcknowledged: true };
    const next = await native(proposal);
    expect(next).toMatchObject({ kind: 'research.cycle.admit.result', state: 'applied', receipt: { ordinal: 1, expectedExecutionRevision: 5, deltaMicros: 1000000 } });
    if (next.kind !== 'research.cycle.admit.result' || !next.receipt) throw Error('Expected exact v2 admission receipt');
    expect(await native(proposal)).toEqual(next);
    const reference = { ordinal: next.receipt.ordinal, admissionFingerprint: next.receipt.fingerprint };
    const execute = { version: 2 as const, kind: 'research.cycle.execute' as const, workspaceId: request.workspaceId, pairingId: request.pairingId, reference };
    expect(await native(execute)).toMatchObject({ kind: 'research.cycle.status.result', authorityState: 'paused', outcome: null });
    expect(ownerResearchSourceSchema.parse(f.dynamo.inspect(ownerResearchSourceKey()))).toEqual(paused);
    expect(historicalKeys.map(key => f.dynamo.inspect(key))).toEqual(historicalRows);
    expect(f.dynamo.inspect('BUDGET#research')).toEqual(researchBefore);
    expect(discovery).toHaveBeenCalledTimes(2); expect(ssm.send).toHaveBeenCalledTimes(2); expect(pageHttp).not.toHaveBeenCalled();
    f.mount(); region = within(await screen.findByRole('region', { name: 'Cloud research' }));
    await region.findByText('Existing policy (read-only): paused');
    expect(region.getByText('Discovery balance: cumulative ceiling $3 USD, reserved-or-spent $2 USD, remaining $1 USD.')).toBeTruthy();
    expect(region.getByText('Research balance: cumulative ceiling $0.01 USD, reserved-or-spent $0 USD, remaining $0.01 USD.')).toBeTruthy();
    expect(f.requests.filter(value => value.path === '/research/setup')).toHaveLength(4);
    await f.assertEncrypted(); await f.unmount(); f.mount();
    region = within(await screen.findByRole('region', { name: 'Cloud research' }));
    await region.findByText('Existing policy (read-only): paused');
    ...
    expect(completed.outcome).toMatchObject({ state: 'completed', settled: true, runId: next.receipt.runId, evidenceReceiptId: expect.any(String), settlementReceiptId: expect.any(String) });
    expect((await native(execute)).outcome).toEqual(completed.outcome);
    expect((await native({ ...statusRequest, reference })).outcome).toEqual(completed.outcome);
    expect(await native({ ...request, kind: 'research.once.status', runId: original.runId! })).toEqual(original);
    expect(await native({ ...request, expectedSourceRevision: 3, kind: 'research.once.status', runId: second.runId! })).toEqual(second);
    expect(historicalKeys.map(key => f.dynamo.inspect(key))).toEqual(historicalRows);
    expect(budgetSchema.parse(f.dynamo.inspect(budgetKey(guidedResearchBudgetId)))).toMatchObject({ limit: 3000000, spent: 3000000 });
    expect(budgetSchema.parse(f.dynamo.inspect('BUDGET#research'))).toMatchObject({ limit: 10000, spent: 10000 });
    expect(discovery).toHaveBeenCalledTimes(3); expect(ssm.send).toHaveBeenCalledTimes(3); expect(pageHttp).toHaveBeenCalledTimes(1);
    f.mount(); region = within(await screen.findByRole('region', { name: 'Cloud research' })); await region.findByText('Existing policy (read-only): active');
    expect(region.getByText('Discovery balance: cumulative ceiling $3 USD, reserved-or-spent $3 USD, remaining $0 USD.')).toBeTruthy();
    expect(f.requests.filter(value => value.path === '/research/setup')).toHaveLength(5);
    expect(discovery).toHaveBeenCalledTimes(3); await f.assertEncrypted();
  } finally { await f.finish(); }
}, 25000);
