...
describe('0017 discovery assessments migration', () => {
  ...
  it('adds exactly schema17 to real16 without rewriting selected business rows, and accepts the exact manifest', async () => {
    ...
    expect(await migrate()).toEqual({ fromVersion: 17, toVersion: 30, appliedMigrationIds: ['0018PlaybookDueActions', '0019EmailDrafts', '0020PmAccounts', '0021DelegatedWork', '0022MailPersistence', '0023Campaigns', '0024RequestedFollowupAndPolicyReviews', '0025KnownCompanyResearchSettings', '0026LocalCompanyDrafts', '0027ListedRouteVerification', '0028TerritoryClearances', '0029AccountCallbacks', '0030EmailTemplates'] });
    ...
    expect(assertDomainStorageReady({ database: f.database, expectedBusyTimeoutMs: 5000,
      expectedSchemaVersion: 30, expectedManifest: DOMAIN_SCHEMA_MANIFEST }).schemaVersion).toBe(30);
    ...
    expect(await migrate()).toEqual({ fromVersion: 30, toVersion: 30, appliedMigrationIds: [] });
  });
