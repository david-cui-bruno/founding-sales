...
describe('0017 discovery assessments migration', () => {
  ...
  it('adds exactly schema17 to real16 without rewriting selected business rows, and accepts the exact manifest', async () => {
    ...
    expect(await migrate()).toEqual({ fromVersion: 17, toVersion: 23, appliedMigrationIds: ['0018PlaybookDueActions', '0019EmailDrafts', '0020PmAccounts', '0021DelegatedWork', '0022MailPersistence', '0023Campaigns'] });
    ...
    expect(assertDomainStorageReady({ database: f.database, expectedBusyTimeoutMs: 5000,
      expectedSchemaVersion: 23, expectedManifest: DOMAIN_SCHEMA_MANIFEST }).schemaVersion).toBe(23);
    ...
    expect(await migrate()).toEqual({ fromVersion: 23, toVersion: 23, appliedMigrationIds: [] });
  });
