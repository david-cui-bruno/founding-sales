...
import { assertPreReleaseStorageReady } from '../../../../src/main/domain/startup/storageReadiness';
import { createTempDatabase, createTestWorkspaceKey } from '../../../fixtures/tempDatabase';

const migrateThrough25 = createMigrationRunner(productionMigrations.filter(entry => entry.schemaVersion <= 25));
...
it('adds only nullable local research and revision0 while preserving genuine24 settings and all other rows', async () => {
  ...
  try {
    ...
    const before = rows();
    expect(await migrateThrough25(database, options)).toEqual({ fromVersion: 24, toVersion: 25, appliedMigrationIds: ['0025KnownCompanyResearchSettings'] });
    ...
    try {
      expect(await migrateThrough25(reopened, options)).toEqual({ fromVersion: 25, toVersion: 25, appliedMigrationIds: [] });
