...
const EXPECTED_MIGRATION_LEDGER = [
  ...
  '0016ContactPresentationEvidence', '0017DiscoveryAssessments', '0018PlaybookDueActions', '0019EmailDrafts', '0020PmAccounts', '0021DelegatedWork', '0022MailPersistence',
] as const;
...
describe('domain startup', () => {
  ...
  describe('storage readiness gate', () => {
    it('passes only the exact schema-22 manifest and ordered migration ledger', () => {
      ...
      expect(readiness).toMatchObject({
        schemaVersion: 22, encrypted: true, ftsAvailable: true,
      });
    ...
    it.each([15, 16, 18, 99])(
      ...
      (schemaVersion) => {
        ...
        expect(caught).toMatchObject({
          ...
          message: 'The workspace schema version is not exactly 22.',
        });
        expect(clockReads()).toBe(0);
        expect(idsUsed()).toBe(0);
      },
    ...
    ])('rejects a $name schema-22 catalog before composition', ({ corrupt }) => {
      corrupt(database);
      const { runtime, clockReads, idsUsed } = buildRuntime();
      expect(() => runtime.initialize()).toThrow(DomainStartupFatalError);
      expect(clockReads()).toBe(0);
