...
describe('domain startup', () => {
  ...
  describe('storage readiness gate', () => {
    it('passes only the exact schema-30 manifest and ordered migration ledger', () => {
      ...
      expect(readiness).toMatchObject({
        schemaVersion: 30, encrypted: true, ftsAvailable: true,
      });
    ...
    it.each([15, 16, 18, 99])(
      ...
      (schemaVersion) => {
        ...
        expect(caught).toMatchObject({
          ...
          message: 'The workspace schema version is not exactly 30.',
        });
        expect(clockReads()).toBe(0);
        expect(idsUsed()).toBe(0);
      },
    ...
    ])('rejects a $name schema-30 catalog before composition', ({ corrupt }) => {
      corrupt(database);
      const { runtime, clockReads, idsUsed } = buildRuntime();
      expect(() => runtime.initialize()).toThrow(DomainStartupFatalError);
      expect(clockReads()).toBe(0);
