...
async function fixture(version = 29, journal = 'WAL') {
...
function fingerprint(raw: driver.RawDatabase) {
  try {
    return readAndVerifyDatabaseFingerprint(raw, [29]);
  } finally {
...
describe('bounded encrypted startup preparation', () => {
  it.each(['WAL', 'DELETE'])(
    ...
    async (journal) => {
      const f = await fixture(29, journal);
  ...
  it.each(['.encryption-state.json', '.encrypting', '.plaintext-recovery-wal'])(
    ...
    async (suffix) => {
      const f = await fixture(29, 'DELETE');
  ...
  it.each([
    ...
    const observation = observePrepare(f, () => {
      ...
      if (kind === 'future-schema')
        raw.exec('UPDATE app_meta SET schema_version=30');
  ...
  it.each([
    ...
    if (kind === 'valid' || kind === 'future') {
      ...
      raw.exec(
        kind === 'future'
          ? 'UPDATE app_meta SET schema_version=30'
