...
async function fixture(version = 26, journal = 'WAL') {
...
function fingerprint(raw: driver.RawDatabase) {
  try {
    return readAndVerifyDatabaseFingerprint(raw, [26]);
  } finally {
...
describe('bounded encrypted startup preparation', () => {
  it.each(['WAL', 'DELETE'])(
    ...
    async (journal) => {
      const f = await fixture(26, journal);
  ...
  it.each(['.encryption-state.json', '.encrypting', '.plaintext-recovery-wal'])(
    ...
    async (suffix) => {
      const f = await fixture(26, 'DELETE');
  ...
  it.each([
    ...
    const observation = observePrepare(f, () => {
      ...
      if (kind === 'future-schema')
        raw.exec('UPDATE app_meta SET schema_version=27');
  ...
  it.each([
    ...
    if (kind === 'valid' || kind === 'future') {
      ...
      raw.exec(
        kind === 'future'
          ? 'UPDATE app_meta SET schema_version=27'
