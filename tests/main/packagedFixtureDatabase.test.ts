...
describe('bounded packaged fixture database preparation', () => {
  ...
  it('creates a current-schema local workspace fixture without pairing, execution authority or a transition', async () => {
    ...
    expect(result.inspection.schemaVersion).toBe(30);
  ...
  it('inspects genuine schema19 independently of current schema28 and refuses catalog drift', async () => {
    const key = createTestWorkspaceKey();
    ...
    await expect(f.inspectStoppedProfile('current', material, 30)).rejects.toThrow(ERROR);
  ...
  it('inspects genuine schema20 independently of current schema28 and refuses catalog drift', async () => {
    const key = createTestWorkspaceKey();
    fs.writeFileSync(dbPath('current'), '', { flag: 'wx', mode: 0o600 });
    fs.writeFileSync(envelopePath('current'), OPAQUE_ENVELOPE, { flag: 'wx', mode: 0o600 });
    const database = openDatabase({ path: dbPath('current'), key });
  ...
  it('inspects genuine schema21 independently of current schema28 and refuses catalog drift', async () => {
    const key = createTestWorkspaceKey();
    fs.writeFileSync(dbPath('current'), '', { flag: 'wx', mode: 0o600 });
    fs.writeFileSync(envelopePath('current'), OPAQUE_ENVELOPE, { flag: 'wx', mode: 0o600 });
    const database = openDatabase({ path: dbPath('current'), key });
    try {
      await migrations.createMigrationRunner(migrations.productionMigrations.filter(entry => entry.schemaVersion <= 21))(
        database, { workspaceKey: key, backupDirectory: join(f.paths.current, 'backups') });
      database.raw.prepare('INSERT INTO persons(id, display_name, created_at, updated_at) VALUES (?, ?, ?, ?)')
        .run('historical21-person', 'FICTIONAL HISTORICAL21', TIME, TIME);
      expect(database.raw.prepare('SELECT schema_version FROM app_meta').get()).toEqual({ schema_version: 21 });
      expect(database.raw.prepare("SELECT name FROM sqlite_master WHERE name='delegated_mail_cursors'").get()).toBeUndefined();
    } finally { closeDatabase(database); key.bytes.fill(0); }
    const before = identity(dbPath('current')); const resources = observeResources();
    const inspected = await f.inspectStoppedProfile('current', material, 21);
    expect(inspected.schemaVersion).toBe(21);
    expect(inspected.catalogSha256).toBe('fb0d87e7930b98b63c0cf896c7d6739a881c5044a11771bec8e211158bc26d29');
    expect(inspected.ledger).toHaveLength(21); expect(inspected.ledger.at(-1)).toBe('0021DelegatedWork');
    expect(inspected.aggregateCounts.people).toBe(1);
    await expect(f.inspectStoppedProfile('current', material, 30)).rejects.toThrow(ERROR);
    expect(identity(dbPath('current'))).toEqual(before); expect(residue()).toEqual([]); resources.assertReleased();
    const corruptKey = createTestWorkspaceKey(); const corrupt = openDatabase({ path: dbPath('current'), key: corruptKey });
  ...
  it('inspects genuine schema22 independently of current schema28 and refuses catalog drift', async () => {
    const key = createTestWorkspaceKey();
    fs.writeFileSync(dbPath('current'), '', { flag: 'wx', mode: 0o600 });
    fs.writeFileSync(envelopePath('current'), OPAQUE_ENVELOPE, { flag: 'wx', mode: 0o600 });
    const database = openDatabase({ path: dbPath('current'), key });
    try {
      await migrations.createMigrationRunner(migrations.productionMigrations.filter(entry => entry.schemaVersion <= 22))(
        database, { workspaceKey: key, backupDirectory: join(f.paths.current, 'backups') });
      database.raw.prepare('INSERT INTO persons(id, display_name, created_at, updated_at) VALUES (?, ?, ?, ?)')
        .run('historical22-person', 'FICTIONAL HISTORICAL22', TIME, TIME);
      expect(database.raw.prepare('SELECT schema_version FROM app_meta').get()).toEqual({ schema_version: 22 });
      expect(database.raw.prepare("SELECT name FROM sqlite_master WHERE name='campaign_versions'").get()).toBeUndefined();
    } finally { closeDatabase(database); key.bytes.fill(0); }
    const before = identity(dbPath('current')); const resources = observeResources();
    const inspected = await f.inspectStoppedProfile('current', material, 22);
    expect(inspected.schemaVersion).toBe(22);
    expect(inspected.catalogSha256).toBe('13e641da99b4e61f4dd7a05a1d07bda21ac642dd3646d5ad35c0a582a7fb106e');
    expect(inspected.ledger).toHaveLength(22); expect(inspected.ledger.at(-1)).toBe('0022MailPersistence');
    expect(inspected.aggregateCounts.people).toBe(1);
    await expect(f.inspectStoppedProfile('current', material, 30)).rejects.toThrow(ERROR);
    expect(identity(dbPath('current'))).toEqual(before); expect(residue()).toEqual([]); resources.assertReleased();
    const corruptKey = createTestWorkspaceKey(); const corrupt = openDatabase({ path: dbPath('current'), key: corruptKey });
    ...
    const corruptBefore = identity(dbPath('current'));
    await expect(f.inspectStoppedProfile('current', material, 22)).rejects.toThrow(ERROR);
    expect(identity(dbPath('current'))).toEqual(corruptBefore); expect(residue()).toEqual([]); resources.assertReleased();
  });

  it('inspects genuine schema23 independently of current schema28 and refuses catalog drift', async () => {
    const key = createTestWorkspaceKey();
    fs.writeFileSync(dbPath('current'), '', { flag: 'wx', mode: 0o600 });
    fs.writeFileSync(envelopePath('current'), OPAQUE_ENVELOPE, { flag: 'wx', mode: 0o600 });
    const database = openDatabase({ path: dbPath('current'), key });
    try {
      await migrations.createMigrationRunner(migrations.productionMigrations.filter(entry => entry.schemaVersion <= 23))(
        database, { workspaceKey: key, backupDirectory: join(f.paths.current, 'backups') });
      database.raw.prepare('INSERT INTO persons(id, display_name, created_at, updated_at) VALUES (?, ?, ?, ?)')
        .run('historical23-person', 'FICTIONAL HISTORICAL23', TIME, TIME);
      expect(database.raw.prepare('SELECT schema_version FROM app_meta').get()).toEqual({ schema_version: 23 });
      expect(database.raw.prepare("SELECT name FROM sqlite_master WHERE name='delegated_requested_followup_drafts'").get()).toBeUndefined();
    } finally { closeDatabase(database); key.bytes.fill(0); }
    const before = identity(dbPath('current')); const resources = observeResources();
    const inspected = await f.inspectStoppedProfile('current', material, 23);
    expect(inspected.schemaVersion).toBe(23);
    expect(inspected.catalogSha256).toBe('53f24bedc785b09d913ee35d89fa7d07ebafa0ebea493efba1660234cc8fb89a');
    expect(inspected.ledger).toHaveLength(23); expect(inspected.ledger.at(-1)).toBe('0023Campaigns');
    expect(inspected.aggregateCounts.people).toBe(1);
    await expect(f.inspectStoppedProfile('current', material, 30)).rejects.toThrow(ERROR);
    expect(identity(dbPath('current'))).toEqual(before); expect(residue()).toEqual([]); resources.assertReleased();
    const corruptKey = createTestWorkspaceKey(); const corrupt = openDatabase({ path: dbPath('current'), key: corruptKey });
    ...
    const corruptBefore = identity(dbPath('current'));
    await expect(f.inspectStoppedProfile('current', material, 23)).rejects.toThrow(ERROR);
    expect(identity(dbPath('current'))).toEqual(corruptBefore); expect(residue()).toEqual([]); resources.assertReleased();
  });
  ...
  it('inspects a normal closed WAL source only through a private encrypted readonly copy and removes copy-side WAL before return', async () => {
    ...
    const result = await f.inspectStoppedProfile('current', material, 30);
    expect(result.aggregateCounts).toEqual({ people: 1, prospects: 1, sourceEvents: 1 });
    expect(result.schemaVersion).toBe(30);
    expect(result.ledger).toHaveLength(30);
    expect(result.ledger.at(-1)).toBe('0030EmailTemplates');
  ...
  it.each(['wrong', 'malformed', 'checksum', 'version'])('rejects %s supplied material with no source mutation or inspection residue', async kind => {
    ...
    await expect(f.inspectStoppedProfile('current', value, 30)).rejects.toThrow(ERROR);
  ...
  it('rejects escaped or unknown selectors without inspecting outside the captured root', async () => {
    const open = vi.spyOn(driver, 'createRawDatabase');
    await expect(f.inspectStoppedProfile('../current' as 'current', material, 30)).rejects.toThrow(ERROR);
  ...
  it('refuses all offline operations and cleanup while the actual captured Node fixture child is alive, then permits observed exit', async () => {
    ...
    expect((await f.inspectStoppedProfile('current', material, 30)).schemaVersion).toBe(30);
  ...
  it('creates a real manual nonzero encrypted backup and repository receipt without changing business data', async () => {
    await seed('current', true);
    const before = await f.inspectStoppedProfile('current', material, 30);
    ...
    expect(result.backup.kind).toBe('manual'); expect(result.backup.schemaVersion).toBe(30);
    ...
    const after = await f.inspectStoppedProfile('current', material, 30);
    expect(after.businessSha256).toBe(before.businessSha256);
    expect(after.backupReceipts).toHaveLength(1);
    expect(after.backupReceipts[0]).toMatchObject({ backup_basename: result.backup.basename, sha256: result.backup.sha256, kind: 'manual', schema_version: 30, size_bytes: result.backup.sizeBytes });
    ...
    const live = await f.inspectStoppedProfile('current', material, 30);
    const backup = await f.inspectStoppedBackup('current', result.backup.basename, material, 30);
  ...
  it('creates separate nonzero through16 and preserves its exact encrypted backup and named subset across17', async () => {
    ...
    try {
      expect(await migrations.migrateToLatest(currentDatabase, { workspaceKey: currentKey, backupDirectory: join(f.paths.through16, 'backups') }))
        .toEqual({ fromVersion: 17, toVersion: 30, appliedMigrationIds: ['0018PlaybookDueActions', '0019EmailDrafts', '0020PmAccounts', '0021DelegatedWork', '0022MailPersistence', '0023Campaigns', '0024RequestedFollowupAndPolicyReviews', '0025KnownCompanyResearchSettings', '0026LocalCompanyDrafts', '0027ListedRouteVerification', '0028TerritoryClearances', '0029AccountCallbacks', '0030EmailTemplates'] });
    ...
    const current = await f.inspectStoppedProfile('through16', material, 30);
    expect(current.ledger.at(-1)).toBe('0030EmailTemplates');
    ...
    expect((await f.inspectStoppedProfile('current', material, 30)).preserved16Sha256).toBeUndefined();
    expect(residue()).toEqual([]);
  });

  it('preserves a nonempty action-independent projection on direct16→30 migration and detects business mutations', async () => {
    ...
    try {
      ...
      expect(await migrations.migrateToLatest(database, { workspaceKey: key, backupDirectory: join(f.paths.through16, 'backups') }))
        .toEqual({ fromVersion: 16, toVersion: 30, appliedMigrationIds: ['0017DiscoveryAssessments', '0018PlaybookDueActions', '0019EmailDrafts', '0020PmAccounts', '0021DelegatedWork', '0022MailPersistence', '0023Campaigns', '0024RequestedFollowupAndPolicyReviews', '0025KnownCompanyResearchSettings', '0026LocalCompanyDrafts', '0027ListedRouteVerification', '0028TerritoryClearances', '0029AccountCallbacks', '0030EmailTemplates'] });
    ...
    const after = await f.inspectStoppedProfile('through16', material, 30);
    ...
    for (const mutation of [
      ...
      const inspected = await f.inspectStoppedProfile('through16', material, 30);
    ...
    expect((await f.inspectStoppedProfile('bootstrap', material, 30)).preservedActionIndependentSha256).toBeUndefined();
  });
  ...
  it.each(['catalog', 'ledger', 'version'])('rejects unsupported %s with fixed safe text and immutable original', async kind => {
    ...
    try {
      ...
      if (kind === 'version') {
        expect(migrations.isRegisteredSchemaVersion(31)).toBe(false);
        database.raw.exec('UPDATE app_meta SET schema_version = 31');
      }
    } finally { closeDatabase(database); key.bytes.fill(0); }
    const before = identity(dbPath('current')); const resources = observeResources();
    await expect(f.inspectStoppedProfile('current', material, 30)).rejects.toThrow(ERROR);
  ...
  it('does not treat an empty current profile as a nonzero manual backup or overwrite a missing target', async () => {
    await seed('current'); const before = identity(dbPath('current'));
    await expect(f.createManualBackup(material)).rejects.toThrow(ERROR);
    await expect(f.inspectStoppedBackup('current', 'manual-20260906T120000000Z.sqlite3', material, 30)).rejects.toThrow(ERROR);
    expect(identity(dbPath('current'))).toEqual(before);
  });
  ...
  it.each([
    ...
    const result = await f.inspectStoppedProfile(profile, material, 30);
    expect(result.aggregateCounts).toEqual({ people: 1, prospects: 1, sourceEvents: 1 });
  ...
  it('creates the actual manual backup from an ordinary owned644 current database without source chmod', async () => {
    ...
    expect(result.backup.sha256).toBe(digest(result.backup.path));
    expect(identity(envelopePath('current'))).toEqual(envelope);
    const current = await f.inspectStoppedProfile('current', material, 30);
  ...
  it.each([
    ...
    const open = vi.spyOn(driver, 'createRawDatabase');
    await expect(f.inspectStoppedProfile(profile, material, 30)).rejects.toThrow(ERROR);
  ...
  it.each(['backup', 'current-envelope', 'bootstrap-envelope', 'historical-envelope', 'historical-db'])('keeps644 forbidden for the %s role', async role => {
    ...
    if (role === 'backup') {
      ...
      inspect = () => f.inspectStoppedBackup('current', result.backup.basename, material, 30);
    } else if (role === 'current-envelope') {
      path = envelopePath('current'); inspect = () => f.inspectStoppedProfile('current', material, 30);
