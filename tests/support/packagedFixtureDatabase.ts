...
type FixtureSchema = 15 | 16 | 17 | 19 | 20 | 21 | 22 | 23 | 25 | 30;
...
function inspectRows(raw: RawDatabase, expected: FixtureSchema, sourceSha256: string, through16: boolean): FixtureInspection {
  if (expected !== 15 && expected !== 16 && expected !== 17 && expected !== 19 && expected !== 20 && expected !== 21 && expected !== 22 && expected !== 23 && expected !== 25 && expected !== 30) fail();
  ...
  return {
    ...
    createHistoricalProfile(material: string) {
      return withMaterial(material, async key => {
        ...
        try {
          ...
          inspect('bootstrap', databasePath('bootstrap'), key, 30); // Proves supplied key matches bootstrap DB, NOT safeStorage.
    ...
    createLocalWorkspaceProfile(material: string) {
      return withMaterial(material, async key => {
        ...
        try {
          if (!same(envelopeCapture.identity, fs.fstatSync(envelope.descriptor)) || envelopeCapture.sha256 !== envelope.sha256) fail();
          inspect('bootstrap', databasePath('bootstrap'), key, 30);
          envelope.assertUnchanged();
          ...
          if (migration.fromVersion !== 0 || migration.toVersion !== 30) fail();
          const seeded = seedLocalWorkspaceAcceptance(database);
          closeDatabase(database); database = undefined;
          return { ...seeded, inspection: inspect('current', databasePath('current'), key, 30) };
        } finally {
          try { if (database) closeDatabase(database); }
          finally { try { envelope.assertUnchanged(); } finally { envelope.close(); } }
        }
    ...
    createManualBackup(material: string) {
      return withMaterial(material, async key => {
        ...
        const before = inspect('current', path, key, 30);
        ...
        try {
          ...
          assertDomainStorageReady({ database, expectedBusyTimeoutMs: 5000, expectedSchemaVersion: 30, expectedManifest: DOMAIN_SCHEMA_MANIFEST });
          ...
          closeDatabase(database); database = undefined;
          const inspection = inspect('current', backupPath('current', backup.basename), key, 30);
          const after = inspect('current', path, key, 30);
