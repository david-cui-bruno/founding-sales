...
type FixtureSchema = 15 | 16 | 17 | 19 | 20 | 21 | 22 | 23 | 24;
...
function inspectRows(raw: RawDatabase, expected: FixtureSchema, sourceSha256: string, through16: boolean): FixtureInspection {
  if (expected !== 15 && expected !== 16 && expected !== 17 && expected !== 19 && expected !== 20 && expected !== 21 && expected !== 22 && expected !== 23 && expected !== 24) fail();
  ...
  return {
    ...
    createHistoricalProfile(material: string) {
      return withMaterial(material, async key => {
        ...
        try {
          ...
          inspect('bootstrap', databasePath('bootstrap'), key, 24); // Proves supplied key matches bootstrap DB, NOT safeStorage.
    ...
    createManualBackup(material: string) {
      return withMaterial(material, async key => {
        ...
        const before = inspect('current', path, key, 24);
        ...
        try {
          ...
          assertDomainStorageReady({ database, expectedBusyTimeoutMs: 5000, expectedSchemaVersion: 24, expectedManifest: DOMAIN_SCHEMA_MANIFEST });
          ...
          closeDatabase(database); database = undefined;
          const inspection = inspect('current', backupPath('current', backup.basename), key, 24);
          const after = inspect('current', path, key, 24);
