...
async function runScenario(): Promise<void> {
  ...
  try {
    if (scenario === 'conversion') {
      ...
      const before = readEncryptedFingerprint(workspace.path, 30);
      await prepareEncryptedDatabase(workspace.path, createTestWorkspaceKey());
      ...
      assert.deepEqual(readEncryptedFingerprint(workspace.path, 30), before);
      assertEncryptedRetainedRow(workspace.path, 30);
      assertArtifactsAbsent(workspace.path);
    ...
    } else if (scenario === 'encrypted-schema-31-rejected') {
      await assertEncryptedSchemaVersionRejected(workspace.path, 31);
