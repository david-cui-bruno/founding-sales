...
async function runScenario(): Promise<void> {
  ...
  try {
    if (scenario === 'conversion') {
      ...
      const before = readEncryptedFingerprint(workspace.path, 26);
      await prepareEncryptedDatabase(workspace.path, createTestWorkspaceKey());
      ...
      assert.deepEqual(readEncryptedFingerprint(workspace.path, 26), before);
      assertEncryptedRetainedRow(workspace.path, 26);
      assertArtifactsAbsent(workspace.path);
    ...
    } else if (scenario === 'encrypted-schema-27-rejected') {
      await assertEncryptedSchemaVersionRejected(workspace.path, 27);
